...
export async function getProductById(id: string): Promise<ProductForEdit | null> {
  ...
  const { data, error } = await supabase
    .from("store_products")
    .select(
      "id, source, category_id, name_en, name_ar, description_en, description_ar, base_price, margin_percent, final_price, merchant_id, merchant_product_id, merchant_products(name_en, merchants(business_name))",
    )
    .eq("id", id)
    .maybeSingle();
